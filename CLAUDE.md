...
2. **2 GB RAM ceiling check** (MANDATORY — auto-runs via hook `hooks/session/13-ram-guard.sh`): This project's Docker stack must not exceed 2 GB total RAM. Hook sums `docker stats` across the project's **default-profile** containers and warns if over. Fix: add `mem_limit` to compose services (suggested split: app 1g + db 512m + sidecars 512m). Exempt a heavy-model repo with `touch .ram-exempt`. **The ceiling covers the APP stack only** — services behind a compose profile (CI infra, heavy sidecars) are deliberately excluded, so total daemon usage for the project can legitimately exceed 2 GB while the guard reads "under ceiling". If the hook can't determine which services are default-profile it counts everything, so it over-reports rather than silently disabling itself.
