...
# Event: SessionStart
# Policy: AI_RULES §12 — node_modules + build artifacts + .git must NEVER sync to Dropbox.
#
# Auto-applies Dropbox's per-folder ignore flag to any node_modules/build/.git dirs
# that have (re)appeared in THIS repo (e.g. after an npm install or git clone), on every
